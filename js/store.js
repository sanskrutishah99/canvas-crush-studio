...
const CCS = {
  ...
  addToCart(artworkId, type, frame) {
    const artwork = this.getArtwork(artworkId);
    if (!artwork) return false;
    const cart = this.getCart();
    const key = `${artworkId}_${type}_${frame || 'none'}`;
    if (cart[key]) {
  ...
  getSettings() {
    try {
      ...
      return { adminPassword: 'canvas2024', stripePaymentLink: '', formspreeId: '', shippingRate: 45, taxRate: 0.1 };
    }
  },
